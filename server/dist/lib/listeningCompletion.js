...
exports.minEngagedSecondsForCompletion = minEngagedSecondsForCompletion;
exports.resolveListeningCompletion = resolveListeningCompletion;
/** Minimum seconds of real playback (client-measured) before we accept "completed". */
function minEngagedSecondsForCompletion(mediaDurationSeconds) {
    if (!Number.isFinite(mediaDurationSeconds) || mediaDurationSeconds < 1)
        return 1;
 ...
 */
function resolveListeningCompletion(args) {
    if (args.previousCompleted)
        return true;
    if (!args.clientWantsComplete)
        return false;
